...
exports.calculateKybRisk = void 0;
...
const isCurrentMonth = (date) => {
    ...
    const now = new Date();
    return (inputDate.getUTCFullYear() === now.getUTCFullYear() &&
        inputDate.getUTCMonth() === now.getUTCMonth());
};
const createRiskFactor = (input) => input;
const getDecision = (score, hasCriticalRisk) => {
    if (hasCriticalRisk)
        return kyb_constants_1.KYB_DECISIONS.HIGH_RISK;
    if (score >= 60)
        return kyb_constants_1.KYB_DECISIONS.HIGH_RISK;
    if (score >= 25)
        return kyb_constants_1.KYB_DECISIONS.REVIEW_REQUIRED;
    return kyb_constants_1.KYB_DECISIONS.SAFE;
};
const calculateKybRisk = (kybCase) => {
    ...
    for (const documentType of REQUIRED_DOCUMENTS) {
        ...
        if (!document) {
            riskFactors.push(createRiskFactor({
                ...
                points: kyb_constants_1.RISK_RULES.MISSING_REQUIRED_DOCUMENT,
                severity: "medium",
                evidence: {
                    documentType,
                },
    ...
    for (const document of documents) {
        ...
        if (isExpired) {
            ...
            riskFactors.push(createRiskFactor({
                ...
                points: kyb_constants_1.RISK_RULES.EXPIRED_DOCUMENT,
                severity: "high",
                evidence: {
     ...
     */
    const csfDocument = findDocument(kyb_constants_1.KYB_DOCUMENT_TYPES.CONSTANCIA_SITUACION_FISCAL);
    if (!csfDocument) {
        riskFactors.push(createRiskFactor({
            code: "MISSING_CSF",
            ...
            severity: "medium",
            evidence: {
                documentType: kyb_constants_1.KYB_DOCUMENT_TYPES.CONSTANCIA_SITUACION_FISCAL,
            },
        }));
    }
        ...
        riskFactors.push(createRiskFactor({
            ...
            description: "La constancia de situación fiscal no corresponde al mes vigente.",
            points: kyb_constants_1.RISK_RULES.CSF_NOT_CURRENT_MONTH,
            severity: "high",
            evidence: {
    ...
    for (const document of documentsWithRfc) {
        if (!isSameText(kybCase.client.rfc, document.extractedRfc)) {
            riskFactors.push(createRiskFactor({
                ...
                points: kyb_constants_1.RISK_RULES.MATERIAL_DISCREPANCY,
                severity: "high",
                evidence: {
                    documentId: document.id,
                    documentType: document.type,
    ...
    const documentsWithLegalName = documents.filter((document) => document.extractedLegalName);
     ...
     */
    const documentsWithAddress = documents.filter((document) => document.extractedAddress);
    ...
    if (!kybCase.client.legalRepresentativeName) {
        riskFactors.push(createRiskFactor({
            ...
            points: kyb_constants_1.RISK_RULES.INCOMPLETE_LEGAL_REPRESENTATIVE,
            severity: "high",
            evidence: {},
        }));
    }
    const documentsWithRepresentative = documents.filter((document) => document.extractedRepresentative);
    for (const document of documentsWithRepresentative) {
        if (!isSameText(kybCase.client.legalRepresentativeName, document.extractedRepresentative)) {
            riskFactors.push(createRiskFactor({
     ...
     */
    const hasShareholders = Array.isArray(kybCase.client.shareholders) &&
        kybCase.client.shareholders.length > 0;
    const hasBeneficialOwner = Boolean(kybCase.client.beneficialOwner);
    const shareholdersDocument = findDocument(kyb_constants_1.KYB_DOCUMENT_TYPES.SOCIOS_ACCIONISTAS);
    const beneficialOwnerDocument = findDocument(kyb_constants_1.KYB_DOCUMENT_TYPES.BENEFICIARIO_CONTROLADOR);
    if (!hasShareholders &&
        !hasBeneficialOwner &&
        ...
        riskFactors.push(createRiskFactor({
            ...
            description: "No se registró información de socios, accionistas o beneficiario controlador.",
            points: kyb_constants_1.RISK_RULES.INCOMPLETE_SHAREHOLDERS_OR_BENEFICIAL_OWNER,
            severity: "medium",
            evidence: {},
        }));
    }
    ...
    if (satChecks.length === 0) {
        riskFactors.push(createRiskFactor({
            ...
            description: "Todavía no se ha ejecutado la revisión contra listas fiscales públicas del SAT.",
            points: kyb_constants_1.RISK_RULES.FISCAL_LIST_REVIEW_OLDER_THAN_3_MONTHS,
            severity: "medium",
            evidence: {
                source: "SAT",
                status: "pending",
        ...
        if (isOlderThanThreeMonths) {
            ...
            riskFactors.push(createRiskFactor({
                ...
                description: "La revisión de listas fiscales SAT tiene más de 3 meses.",
                points: kyb_constants_1.RISK_RULES.FISCAL_LIST_REVIEW_OLDER_THAN_3_MONTHS,
                severity: "medium",
                evidence: {
        ...
        for (const check of satMatches) {
            ...
            const isCritical = (source === "SAT_ART_69B" && listType === "definitivos") ||
            ...
            if (isCritical) {
                ...
                riskFactors.push(createRiskFactor({
                    ...
                    points: kyb_constants_1.RISK_RULES.SAT_LIST_MATCH_CRITICAL,
                    severity: "critical",
                    evidence: {
                        checkId: check.id,
                        source,
                ...
                riskFactors.push(createRiskFactor({
                    ...
                    points: kyb_constants_1.RISK_RULES.SAT_LIST_MATCH_REVIEW,
                    severity: "high",
                    evidence: {
                        checkId: check.id,
                        source,
