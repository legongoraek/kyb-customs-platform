...
export function CaseDetailPage() {
      ...
      <div className="flex min-h-[65vh] items-center justify-center px-4">
        <div className="w-full max-w-lg rounded-3xl border border-slate-200 bg-white p-8 text-center shadow-xl">
          ...
          <h2 className="text-xl font-black text-slate-900">
            Cargando expediente KYB
          </h2>

          <p className="mt-2 text-sm leading-6 text-slate-500">
            Estamos obteniendo los datos del expediente, documentos, evidencia SAT
            y audit log.
          </p>

          <div className="mt-7 space-y-4 text-left">
            <div className="rounded-2xl border border-slate-100 bg-slate-50 p-4">
              <div className="mb-3 h-3 w-32 animate-pulse rounded-full bg-slate-200" />
    ...
    return (
      <div className="flex min-h-[60vh] items-center justify-center px-4">
        <div className="w-full max-w-md rounded-3xl border border-red-100 bg-white p-8 text-center shadow-xl">
          ...
          <p className="mt-2 text-sm leading-6 text-slate-500">
            No pudimos encontrar el expediente solicitado o ya no está disponible.
          </p>
        </div>
      </div>
    );
  ...
  const isRunningSat = actionLoading === "sat";
  const isRunningRisk = actionLoading === "risk";
  const isApproving = actionLoading === "approve";
  const isAnyActionLoading = actionLoading !== null;

  const getActionMessage = () => {
    if (isRunningSat) {
      return "Consultando listas fiscales SAT y guardando evidencia auditable...";
    }

    if (isRunningRisk) {
      return "Calculando score KYB, factores de riesgo y decisión del expediente...";
    }

    if (isApproving) {
      return "Validando aprobación del expediente...";
    }
    ...
    <div className="space-y-7">
      <section className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
        ...
        <div className="mt-6 rounded-3xl border border-slate-200 bg-slate-50 p-5">
          <div className="flex flex-wrap items-start justify-between gap-3">
            <div>
              ...
              <p className="mt-1 text-sm text-slate-500">
                Ejecuta validaciones, calcula riesgo y exporta evidencia del expediente.
              </p>
            </div>
          ...
          {getActionMessage() && (
            <div className="mt-4 flex items-start gap-3 rounded-2xl border border-blue-100 bg-blue-50 p-4 text-sm text-blue-800">
              <span className="mt-0.5 h-4 w-4 shrink-0 animate-spin rounded-full border-2 border-blue-300 border-t-blue-800" />
              <p className="font-semibold">{getActionMessage()}</p>
            </div>
          )}
            ...
            <div className="flex-1">
              ...
              <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-3">
                <button
                  ...
                  className={`inline-flex min-h-[52px] w-full items-center justify-center gap-2 rounded-2xl px-5 py-3 text-sm font-bold text-white transition ${
                    isRunningSat
                ...
                >
                  ...
                  {isRunningSat ? "Consultando SAT..." : "Consultar SAT"}
                </button>
                ...
                <button
                  ...
                  className={`inline-flex min-h-[52px] w-full items-center justify-center gap-2 rounded-2xl px-5 py-3 text-sm font-bold text-white transition ${
                    isRunningRisk
                      ? "bg-slate-950 shadow-lg shadow-slate-300"
                ...
                >
                  ...
                  {isRunningRisk ? "Calculando score..." : "Ejecutar score KYB"}
                </button>

                <button
                  onClick={handleApprove}
                  disabled={isAnyActionLoading || !canApproveNow}
                  className={`inline-flex min-h-[52px] w-full items-center justify-center gap-2 rounded-2xl px-5 py-3 text-sm font-bold text-white transition ${
                    isApproving
              ...
              {!canApproveNow && (
                <p className="mt-3 flex items-start gap-1.5 rounded-2xl bg-white p-3 text-xs leading-5 text-slate-500 ring-1 ring-slate-200">
                  <Info size={14} className="mt-0.5 shrink-0" />
                  <span>
                    Ejecuta el score KYB con resultado{" "}
                    <span className="font-semibold text-slate-700">safe</span> para
                    poder aprobar este expediente.
                  </span>
                </p>
              )}
            </div>
            ...
            <div className="lg:w-64">
              ...
              <div className="grid grid-cols-2 gap-3 lg:grid-cols-1">
                <a
                  ...
                  target="_blank"
                  rel="noreferrer"
                  className={`inline-flex min-h-[52px] items-center justify-center gap-2 rounded-2xl bg-white px-5 py-3 text-sm font-bold text-slate-700 ring-1 ring-slate-200 transition hover:bg-slate-50 ${
                    isAnyActionLoading ? "pointer-events-none opacity-60" : ""
                  }`}
                >
                  <Download size={16} />
                  JSON
